...
function getGrowthCost() {
  return 80 + state.growthLevel * 80;
}
...
function getFailureReward() {
  return Math.floor(getFullLevelReward() * (state.round / state.maxRounds) * 0.6);
}
...
function renderHud() {
  ...
  const skillReady = state.phase === "combat" && state.heroSkillCooldown <= 0;
  const skillProgress = Math.max(0, Math.min(1, 1 - state.heroSkillCooldown / HERO_SKILL_COOLDOWN));
  ...
  heroSkillTimer.textContent = state.heroSkillCooldown > 0 ? `${Math.ceil(state.heroSkillCooldown)}s` : "大招";
...
function renderHomeHud() {
  ...
  homeGoldText.textContent = state.gold;
  homeStaminaText.textContent = `${state.stamina}/${state.maxStamina}`;
  homeGrowthText.textContent = state.growthLevel + 1;
...
function showHomeFeature(feature) {
  const details = {
    "边塞商店": "边塞商店筹备中。",
    "武将名册": `已集结 3 名武将，当前统帅等级 ${state.growthLevel + 1}。`,
    "角色成长": `当前养成等级 ${state.growthLevel}/${MAX_GROWTH_LEVEL}，通关奖励可用于提升全队战力。`,
    "行军包裹": "行军包裹暂未开放。",
    "每日任务": "每日任务暂未开放。",
...
function releaseHeroSkill() {
  if (state.phase !== "combat" || state.heroSkillCooldown > 0) return;
  state.heroSkillCooldown = HERO_SKILL_COOLDOWN;
  state.monsters.forEach((monster) => {
    const damage = Math.max(55, monster.maxHp * 0.55)
      * state.damageMultiplier
      * (1 + state.growthLevel * 0.2);
    monster.hp -= damage;
    monster.rootRemaining = Math.max(monster.rootRemaining || 0, 2);
    damageBurstAtMonster(monster, `-${Math.round(damage)}`, true);
  });
  const fx = document.createElement("div");
  fx.className = "card-screen-fx hero-skill-fx";
  fxLayer.appendChild(fx);
  setTimeout(() => fx.remove(), 620);
  removeDefeatedMonsters();
  tipText.textContent = "主角释放大招，场上怪物受到重击并被禁锢。";
  renderHud();
  renderMonsters();
}
...
function tick(dt) {
  ...
  state.heroSkillCooldown = Math.max(0, state.heroSkillCooldown - dt);
  state.monsters.forEach((monster) => {
...
function attackMonsters(dt) {
  ...
  units.forEach((unit) => {
    ...
    if (attacked) {
      piece.attackCooldown = ATTACK_INTERVAL_BY_TIER[level] / state.attackSpeedMultiplier;
...
function upgradeGrowth() {
  if (state.phase !== "settle" || state.resolving || state.growthLevel >= MAX_GROWTH_LEVEL) return false;
  const cost = getGrowthCost();
  if (state.gold < cost) return false;
  state.gold -= cost;
  state.growthLevel += 1;
  state.maxHp = 4 + state.growthLevel;
  state.hp = state.maxHp;
  tipText.textContent = `养成完成，武将伤害提高 ${state.growthLevel * 20}%，防线上限提升。`;
  renderHud();
  return true;
}
...
function renderModalArtwork(title) {
  ...
  if (title === "局外养成" || title === "角色成长") stats = [[`${state.growthLevel}/${MAX_GROWTH_LEVEL}`, "养成等级"], [state.gold, "持有金币"]];
  if (title === "胜利结算") stats = [[`+${state.paidReward}`, "通关金币"], [state.totalKills, "本局击败"]];
  if (title === "防线失守" || title === "本关失败") stats = [[`+${state.paidReward}`, "本局金币"], [`${state.round}/${state.maxRounds}`, "到达波次"]];
...
function showModal(title, body, actions) {
  modalCard.classList.remove("card-draft");
...
function showDefeat() {
  const reward = payLevelReward(getFailureReward());
  const rewardText = `本次失败奖励 ${reward} 金币（全额奖励 ${getFullLevelReward()} × 当前第 ${state.round}/${state.maxRounds} 波 × 60%）。`;
  if (!state.revived) {
...
function showVictory() {
  ...
  const reward = payLevelReward(getFullLevelReward());
  const body = `通关成功，获得全额关卡奖励 ${state.paidReward} 金币，本次补发 ${reward} 金币。可在结算页进行局外养成、模拟看广告双倍结算，或进入下一关。`;
  ...
  showModal("胜利结算", body, [
    ...
    { label: "局外养成", secondary: true, onClick: () => showGrowthModal(showVictory, showVictory) },
    { label: "下一关", secondary: true, onClick: nextLevel },
    { label: "返回主界面", secondary: true, onClick: completeLevelToHome },
  ]);
}

function showGrowthModal(onContinue, onBack) {
  const cost = getGrowthCost();
  showModal(
    "局外养成",
    `当前养成等级 ${state.growthLevel}/${MAX_GROWTH_LEVEL}。每次养成消耗 ${cost} 金币，武将伤害提高 20%，防线上限提高 1 点。养成只在关卡结算页生效。`,
    [
      {
        label: state.growthLevel >= MAX_GROWTH_LEVEL ? "养成已满级" : `金币养成（${cost}G）`,
        disabled: state.growthLevel >= MAX_GROWTH_LEVEL || state.gold < cost,
        onClick: () => {
          upgradeGrowth();
          showGrowthModal(onContinue, onBack);
        },
      },
      { label: "继续", onClick: onContinue },
      { label: "返回结算", secondary: true, onClick: onBack },
    ],
  );
}
