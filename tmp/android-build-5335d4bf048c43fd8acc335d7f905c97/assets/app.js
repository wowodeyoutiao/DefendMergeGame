...
function getHeroSkillStatus() {
  ...
  if (currentView !== "battle" || state.phase === "setup") {
    ...
    label = "结束";
    message = "本局已结束";
  } else if (state.phase !== "combat" || state.cardQueued || state.hintAd) {
    label = "暂停";
    message = "战斗暂停中";
  } else if (cooldown > 0) {
...
function renderHud() {
  ...
  mergeHintCount.textContent = `剩余${hintsRemaining}次`;
  ...
  battleHomeBtn.disabled = Boolean(state.hintAd);
  resetBtn.disabled = Boolean(state.hintAd);
  speedBtn.disabled = Boolean(state.hintAd);
...
function canRequestMergeHint() {
  return currentView === "battle" && state.levelStaminaSpent
    && state.phase === "setup"
    && !state.resolving && !state.hintAd && !state.hintIndices.length
    && state.mergeHintsUsed < MAX_MERGE_HINTS;
}

function findHintMoves() {
  const existingMatches = collectAllMatches();
  ...
  for (let from = 0; from < state.board.length; from += 1) {
    ...
    for (let to = 0; to < state.board.length; to += 1) {
      const target = state.board[to];
      if (from === to || (target && source.type === target.type
...
function spawnMonster() {
  ...
  const c = isBoss && Number.isInteger(state.bossLane)
    ? state.bossLane
...
function moveMonsters() {
  ...
  const leaked = state.monsters.filter((monster) => monster.y >= BOARD_SIZE);
  if (leaked.length) {
    state.hp = Math.max(0, state.hp - leaked.length);
    state.monsters = state.monsters.filter((monster) => monster.y < BOARD_SIZE);
...
function checkCombatEnd() {
  ...
  if (state.spawned >= state.waveConfig.count && state.monsters.length === 0) {
    ...
    if (state.round >= state.maxRounds) {
      ...
      state.phase = "setup";
      state.steps = 8;
      if (state.round === state.maxRounds) prepareBossWarning();
      addWaveSupply();
...
function showStaminaRefill() {
  ...
  showModal(
    ...
    [
      {
        label: state.staminaPurchaseUsed ? "今日元宝购买已用" : `元宝购买满体力（${STAMINA_PURCHASE_COST}元宝）`,
        disabled: state.staminaPurchaseUsed || state.yuanbao < STAMINA_PURCHASE_COST || atLimit,
        onClick: () => refillStamina("yuanbao"),
      },
      {
        label: state.staminaAdUsed ? "今日广告补充已用" : "观看广告补充满体力",
...
function showMergeHintAd() {
  ...
  const ad = {
    ...
    elapsed: 0,
    timer: null,
    animations: gameScreen.getAnimations({ subtree: true })
      .filter((animation) => animation.playState === "running"),
  };
  state.hintAd = ad;
  state.phase = "hint-ad";
  stopLoop();
  ad.animations.forEach((animation) => animation.pause());
  renderHud();
  showModal(
    "激励广告（试玩）",
    `棋盘提示 · 本局剩余 ${MAX_MERGE_HINTS - state.mergeHintsUsed} 次`,
    [
      { label: "广告播放中 · 3秒", disabled: true, onClick: () => finishMergeHintAd(true) },
      { label: "关闭广告", secondary: true, onClick: () => finishMergeHintAd(false) },
    ],
  );
  const progress = document.createElement("div");
  progress.className = "hint-ad-progress";
  progress.setAttribute("role", "progressbar");
  progress.setAttribute("aria-label", "广告播放进度");
  progress.setAttribute("aria-valuemin", "0");
  progress.setAttribute("aria-valuemax", String(HINT_AD_DURATION_MS));
  progress.setAttribute("aria-valuenow", "0");
  const fill = document.createElement("span");
  progress.appendChild(fill);
  modalDetail.appendChild(progress);
  const rewardButton = modalActions.querySelector("button");
  let lastTime = performance.now();
  ad.timer = setInterval(() => {
    if (state.hintAd !== ad) return;
    const now = performance.now();
    if (!document.hidden) ad.elapsed = Math.min(HINT_AD_DURATION_MS, ad.elapsed + Math.min(250, now - lastTime));
    lastTime = now;
    progress.setAttribute("aria-valuenow", String(Math.floor(ad.elapsed)));
    fill.style.width = `${ad.elapsed / HINT_AD_DURATION_MS * 100}%`;
    const remaining = Math.ceil((HINT_AD_DURATION_MS - ad.elapsed) / 1000);
    rewardButton.textContent = remaining > 0 ? `广告播放中 · ${remaining}秒` : "领取提示";
    rewardButton.disabled = remaining > 0;
    if (remaining <= 0) clearInterval(ad.timer);
  }, 100);
}
